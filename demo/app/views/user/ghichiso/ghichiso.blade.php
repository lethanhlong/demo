...
@section('title')
	<title>Ghi chi số điện tiêu thụ</title>
	...
	$(document).ready(function(){
			 ...
			 // $("#bang1 :checkbox").click(function (evt){
				// 	$("#bang1 :checkbox").prop('checked',false);
				// 	$(this).prop('checked',true);
				// 	document.getElementById("new").disabled=false;
											
				// });
				// if($("#bang1 input:checked").length==0){
...
@section('main-content')
	<div> 
		<div id="thongbao" class="alert-success">Danh sách khách hàng ghi chỉ số điện</div>
		<br>
		<div id="cnhopdong">
			...
			<table class="display" id="bang1">
				...
				<tbody>
					...
					$hopdong = DB::table('hopdongs')
								 ->join('khachhangs', function($join)
						        {
								 ...
								 ->join('tieuthus', function($join)
								 	{
						            $join->on('congtos.socongto', '=', 'tieuthus.socongto');
						        })
								->where('hopdongs.tenloai','=','điện')
								->distinct()
								->get();
					?>
					@foreach ($hopdong as $h)
						...
						?>
						@if($h->thang <= $new_date && $h->thang > $n_date && $h->ttghi == 0 )
					<tr>
